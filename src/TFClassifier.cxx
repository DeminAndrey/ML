...
void TfClassifier::delete_tf_session(TF_Session *tf_session) {
  ...
  if (TF_GetCode(status.get()) != TF_OK) {
    std::stringstream ss;
    ss << " Unable to delete TF_Session: "
...
TfClassifier::TfClassifier(const std::string& modelpath,
  ...
  session_.reset(TF_LoadSessionFromSavedModel(session_opts_.get(),
                                              RunOpts, modelpath.c_str(),
                                              &tags, 1, graph_.get(),
                                              nullptr, status.get()));
  if (TF_GetCode(status.get()) != TF_OK) {
    std::stringstream ss;
    ss << " Unable to import graph from '"
       << modelpath << "': "
       << TF_Message(status.get());
    throw std::invalid_argument{ss.str()};
  }
  size_t pos = 0;
  input_op_ = TF_GraphOperationByName(graph_.get(),
                                      "serving_default_input");
  if (input_op_ == nullptr) {
    throw std::runtime_error{"Input not found"};
  }

  output_op_ = TF_GraphOperationByName(graph_.get(),
...
TfClassifier::probas_t TfClassifier::predict_proba(const features_t& feat) const {
  ...
  tf_tensor input{TF_NewTensor(TF_FLOAT, in_dims, 4,
                               reinterpret_cast<void*>(preproc_features.data()),
                               num_bytes_in, &dummy_deleter, 0),
        TF_DeleteTensor};
  ...
  tf_tensor output_value{TF_AllocateTensor(
          TF_FLOAT, out_dims, 2, num_bytes_out), TF_DeleteTensor};
